...
A continuación compararemos las caracteristicas de los Adelie en las diferentes islas para ver si la zona demográfica influye en la evolución de la especie a nivel fisiológico. Empezaremos con la longitud de sus aletas.
...
```{r}
# Comentario sobre los gráficos

```

Vamos a ver si existe una variación aparentemente significativa en el peso corporal de la especie Adelie dependiendo de las islas. Esto podría ocurrir por distintos factores como: la dificultad que tienen los pingüinos para alimentarse debido a los depredadores; la distancia entre sus colonias y el lugar en el que cazan; o por posibles variaciones físicas.
...
pie(table(penguins$sex[penguins$species == "Adelie"]), main = "Sexos de Adelie",col = "#7EC3E5")

pie(table(penguins$sex[penguins$species == "Chinstrap"]), main = "Sexos de Chinstrap",col = "#E57E7E")

pie(table(penguins$sex[penguins$species == "Gentoo"]), main = "Sexos de Gentoo",col = "#8F7EE5")
...
boxplot(flipper_length_species_sex, names = c("A-F", "A-M", "C-F", "C-M", "G-F", "G-M"), ylab = "Longitud aleta (mm)", xlab = "Especies y sexo", main = "Longitud de las aletas por especies y sexo",  col =c("#7EC3E5", "#7EC3E5", "#E57E7E", "#E57E7E", "#8F7EE5", "#8F7EE5"))
```

En este gráfico podemos observar que la especie Gentoo es la que tiene la longitud de las aletas más larga y que los machos también tienen las aletas más largas que las hembras en las tres especies. También podemos observar una similitud en esta variable entre los Adelies machos y los Chinstrap hembras.

```{r, echo=FALSE, fig.width=8, fig.height=5, fig.align='center'}
par(mfrow = c(1, 1))

bill_length_species_sex = split(penguins2$bill_length_mm, list(penguins2$sex, penguins2$species))
boxplot(bill_length_species_sex, names = c("A-F", "A-M", "C-F", "C-M", "G-F", "G-M"), ylab = "Longitud pico (mm)", xlab = "Especies y sexo", main = "Longitud del pico por especies y sexo",  col =c("#7EC3E5", "#7EC3E5", "#E57E7E", "#E57E7E", "#8F7EE5", "#8F7EE5"))
```
...
boxplot(bill_depth_species_sex, names = c("A-F", "A-M", "C-F", "C-M", "G-F", "G-M"), ylab = "Profundidad pico (mm)", xlab = "Especies y sexo", main = "Profundidad del pico por especies y sexo",  col =c("#7EC3E5", "#7EC3E5", "#E57E7E", "#E57E7E", "#8F7EE5", "#8F7EE5"))
```
...
Volvemos a ver también que los machos toman valores más altos en las tres especies.

```{r, echo=FALSE, fig.width=8, fig.height=5, fig.align='center'}
par(mfrow = c(1, 1))

body_mass_species_sex = split(penguins2$body_mass_g, list(penguins2$sex, penguins2$species))
boxplot(body_mass_species_sex, names = c("A-F", "A-M", "C-F", "C-M", "G-F", "G-M"), ylab = "Masa corporal (g)", xlab = "Especies y sexo", main = "Masa corporal por especies y sexo",  col =c("#7EC3E5", "#7EC3E5", "#E57E7E", "#E57E7E", "#8F7EE5", "#8F7EE5"))
```
Respecto a la masa corporal, se vuelve a dar el caso que la especie Gentoo toma los valores más altos. También vemos ciertas similituddes entre las especies Adelie y Chinstrap. Vemos otra vez que los machos toman valores más altos que las hembras en las tres especies.

```{r comentarios}
# Algun hist de frecuencias absolutas?
# Hay que comentar los valores atípicos?
